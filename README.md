...
「LSB 隱寫」Tab 會把秘密圖片的原始檔案 bytes 逐 bit 寫入載體圖片 RGB 通道的最低有效位元。封包包含版本、圖片格式、尺寸、資料長度與 CRC-32，提取時會驗證完整性。

- 支援 JPEG、PNG、WebP、BMP、GIF、TIFF 與其他 Pillow 可辨識的圖片；提取後的檔案 bytes 與原檔完全相同。
- 外層 LSB 載體輸出固定為 PNG，秘密圖片則保留原始格式。
- 可用容量約為 `載體寬 × 載體高 × 3 ÷ 8`，再扣除 53 bytes 封包標頭。
