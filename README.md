...
程式在 YCbCr 的亮度通道上，以每個 8×8 區塊的兩對中頻 DCT 係數差進行量化索引調變（QIM）。資料先經 BCH(63,45) 糾錯碼，再分散並重複寫入；每個 BCH 區塊可修正最多 3 個錯誤位元，最後用 CRC-32 驗證完整內容。相較舊版 rate-1/2 卷積碼，同一張圖片約可增加 40% 的文字容量；讀取功能仍相容舊版浮水印。
